...
// OnUpdate must be called when the node information was updated
//
...
func (n *Node) OnUpdate() {
	...
	if n.Routing == nil {
		...
		// All code below depends on routing information being provided
		return
	}
	...
	if GetLocalNode().Routing.DirectRouting.InstallRoutes {
		if n.Routing.DirectRouting.Announce {
			updateIPRoute(oldNode, n)
		} else {
			if oldNode != nil && oldNode.Routing != nil && oldNode.Routing.DirectRouting.Announce {
...
func DeleteNode(ni Identity) {
	...
	if n, ok := clusterConf.nodes[ni]; ok {
		...
		n.getLogger().Info("Removed node from cluster")
	}
}
