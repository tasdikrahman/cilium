...
func (d *Daemon) EnableK8sWatcher(reSyncPeriod time.Duration) error {
	...
	if viper.GetBool(k8sUseNodeAnnotations) {
...
func (d *Daemon) addK8sNodeV1(k8sNode *v1.Node) {
	...
	n.Routing = d.conf.routingConfig
	n.OnUpdate()

	log.WithFields(logrus.Fields{
		logfields.K8sNodeID:     k8sNode.ObjectMeta.Name,
...
func (d *Daemon) updateK8sNodeV1(_, k8sNode *v1.Node) {
	...
	if newNode.Name == node.GetLocalNode().Name {
		return
	}
